...
function CardGoal(props) {
  ...
  // Jika data tidak ada atau tidak lengkap, tampilkan loading
  if (!data || data.present_amount === undefined || data.target_amount === undefined) {
    return (
      <Card
        title="Goals"
        desc={
          <div className="flex flex-col justify-center items-center h-full py-12">
            <CircularProgress 
              ...
              sx={{ color: '#299D91' }}
            />
            <div className="mt-4 text-gray-500 font-medium">Loading Data</div>
          </div>
        }
      />
    );
